...
int match,mis_match;	//match和mis_match为比对的预设参数, 再text.txt中被指定
...
/////////////////////////////////////!!!!打分矩阵计算开始!!!!////////////////////////////////////
		...
		for(seq1_id=r_s; seq1_id<=r_e; seq1_id++)	//利用该循环完成所有序列的计算
		{
			for(seq2_id=0; seq2_id<seq_num; seq2_id++)	
			{
				...
				if(col<=124*64)				//计算完成列计算所需要的从核周期数 (124也为预设变量, 124是根据从核局存的大小而设定好的)
				{
					loop_c=(col/124+1)*2-1;	//若列数<=124*63，则不足以发动所有的从核进行计算
				}
				else 
				...
				// 	}
				// }
				// if(seq1_id==5&&seq2_id==0)	//打印指定次数的loop_r/c
				// {
				// 	printf("(5,0): loop_r:%d; loop_c:%d\n", loop_r, loop_c);
				// }
				// if(seq1_id==0&&seq2_id==5)
				// {
				// 	printf("(0,5): loop_r:%d; loop_c:%d\n", loop_r, loop_c);
				// }
				// int is_r, is_c;
				// for(is_r=0;is_r<=row;is_r++)		
				// {
